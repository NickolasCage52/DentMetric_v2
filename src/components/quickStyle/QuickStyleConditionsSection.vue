      ...
      <div class="card-metallic rounded-xl" style="padding:var(--qc-card-py,12px) var(--qc-card-px,12px)">
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          <button
            ...
            :class="model.repairCode ? 'bg-[#1a1a1a] border-metric-green/40' : 'bg-[#151515] border-white/10 hover:border-white/15'"
            @click="$emit('pick', 'repairCode', 'Технология ремонта', repairOptions)"
          >
            <div class="min-w-0 flex-1">
              <div class="qc-sr-value text-[12px] font-semibold truncate" :class="model.repairCode ? 'text-white' : 'text-gray-400'">{{ getRepairLabel(model.repairCode) || 'Без покраски' }}</div>
            </div>
          ...
          <button
            ...
            class="qc-select-row w-full flex items-center justify-between gap-2 border transition-colors touch-manipulation"
            :class="model.riskCode ? 'bg-[#1a1a1a] border-metric-green/40' : 'bg-[#151515] border-white/10 hover:border-white/15'"
          ...
          <button
            ...
            class="qc-select-row w-full flex items-center justify-between gap-2 border transition-colors touch-manipulation"
            :class="model.materialCode ? 'bg-[#1a1a1a] border-metric-green/40' : 'bg-[#151515] border-white/10 hover:border-white/15'"
          ...
          <button
            ...
            class="qc-select-row w-full flex items-center justify-between gap-2 border transition-colors touch-manipulation"
            :class="model.carClassCode ? 'bg-[#1a1a1a] border-metric-green/40' : 'bg-[#151515] border-white/10 hover:border-white/15'"
      ...
      <div class="card-metallic rounded-xl" style="padding:var(--qc-card-py,12px) var(--qc-card-px,12px)">
        ...
        <div style="display:flex;flex-direction:column;gap:4px">
          <button
            ...
            class="qc-select-row w-full flex items-center justify-between gap-2 border transition-colors touch-manipulation"
            :class="(model.disassemblyCodes?.length ?? 0) > 0 ? 'bg-[#1a1a1a] border-metric-green/40' : 'bg-[#151515] border-white/10 hover:border-white/15'"
          ...
          <button
            ...
            class="qc-select-row w-full flex items-center justify-between gap-2 border transition-colors touch-manipulation"
            :class="model.paintMaterialCode ? 'bg-[#1a1a1a] border-metric-green/40' : 'bg-[#151515] border-white/10 hover:border-white/15'"
          ...
          <button
            ...
            class="qc-select-row w-full flex items-center justify-between gap-2 border transition-colors touch-manipulation"
            :class="model.soundInsulationCode ? 'bg-[#1a1a1a] border-metric-green/40' : 'bg-[#151515] border-white/10 hover:border-white/15'"
...
const conditionsComplete = computed(() => {
  const m = props.model;
  if (!m) return false;
  return !!(m.repairCode && m.riskCode && m.materialCode && m.carClassCode && (m.disassemblyCodes?.length ?? 0) > 0);
});
