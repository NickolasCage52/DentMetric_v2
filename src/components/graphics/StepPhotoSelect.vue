<template>
  <div class="photo-select-screen flex flex-col min-h-0">
    <div class="graphics-panel-content p-2 space-y-3">
      <div class="photo-select-title text-white font-bold text-[13px]">Добавьте фото повреждения</div>
      <div class="photo-select-subtitle text-[11px] text-gray-400">
        Сфотографируйте вмятину или выберите из галереи
      </div>

      <div class="photo-select-actions grid grid-cols-2 gap-2">
        <button
          type="button"
          class="photo-btn card-metallic p-4 rounded-xl flex flex-col items-center gap-2 min-h-[80px] active:scale-95 hover:border-metric-green/50 transition-all touch-manipulation"
          @click="openCamera"
        >
          <svg class="w-8 h-8 text-metric-green" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <span class="text-[11px] font-semibold text-white">Камера</span>
        </button>
        <button
          type="button"
          class="photo-btn card-metallic p-4 rounded-xl flex flex-col items-center gap-2 min-h-[80px] active:scale-95 hover:border-metric-green/50 transition-all touch-manipulation"
        ...
        >
          ...
          <span class="text-[11px] font-semibold text-white">Из галереи</span>
        </button>
      </div>
      ...
      <div v-if="photoPreviewUrl" class="photo-preview-wrap rounded-xl overflow-hidden border border-white/10">
        <img :src="photoPreviewUrl" class="photo-preview-img w-full h-auto max-h-[160px] object-contain bg-black/50" alt="Фото повреждения" />
        <button
          type="button"
          class="photo-replace-btn w-full py-2 text-[11px] font-semibold text-metric-green bg-black/30 hover:bg-black/50 transition-colors"
          @click="replacePhoto"
        >
          Заменить фото
        </button>
    ...
    <div class="graphics-action-bar wizard-step-controls space-y-2">
      <div class="flex items-center gap-2 w-full">
        ...
        <button
          ...
          @click="$emit('next')"
          :disabled="!photoAsset"
          :class="photoAsset ? 'bg-metric-green text-black shadow-[0_0_15px_rgba(136,229,35,0.4)]' : 'bg-white/10 text-gray-500 cursor-not-allowed'"
          class="flex-1 py-2.5 rounded-xl font-bold text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 min-h-[44px]"
        >
          Продолжить
        </button>
      </div>
