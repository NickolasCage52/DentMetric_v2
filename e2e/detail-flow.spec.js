/**
 * E2E: Detail mode — photo-based flow.
 */
...
test.describe('Detail flow', () => {
  test('Launch -> go Detail -> add oval -> set sizes -> choose coefficients -> see total', async ({ page }) => {
    ...
    if (!(await galleryBtn.isVisible({ timeout: 2000 }).catch(() => false))) {
      test.skip(true, 'Photo step not shown — may need client step first');
      return;
    }
